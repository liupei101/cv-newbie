...
# input image is with shape of [227, 227, 3]
...
with tf.name_scope("train"):
	...
	train_op = opt.apply_gradients(grads_and_vars=grads_and_vars)
...
train_steps_per_epoch = np.floor(train_generator.data_size / batch_size).astype(np.int16)
val_steps_per_epoch = np.floor(val_generator.data_size / batch_size).astype(np.int16)
...
with tf.Session() as sess:
	...
	for epoch in range(num_epochs):
        ...
        print("[{}] Finished this epoch and start validation".format(datetime.now()))
        test_acc = 0.
        test_count = 0
